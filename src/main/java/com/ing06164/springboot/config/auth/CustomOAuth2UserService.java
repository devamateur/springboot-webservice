...
import com.ing06164.springboot.config.auth.dto.OAuthAttributes;
import com.ing06164.springboot.config.auth.dto.SessionUser;
import com.ing06164.springboot.domain.user.User;
import com.ing06164.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // registrationId: 로그인 중인 서비스를 구분(여기서는 구글 or 네이버)
        String registrationId = userRequest.getClientRegistration().getRegistrationId();

        // userNameAttributeName: OAuth2 로그인 시 키가 되는 필드값(=primary key)
